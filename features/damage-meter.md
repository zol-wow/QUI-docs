...
- A visible session button plus header right-click options to switch type / session
...
- **Session** -- choose `Current`, `Overall`, or one of the 20 most recent Blizzard-tracked combat sessions. Historical fights are named, timed, and marked active in the visible session-button menu; their selection is runtime-only and clears on reload or reset.
...
| Hover Tooltip Scale | Scales the rich hover breakdown from 50%-150%. |
| Show All Hover Spells | Shows up to 15 spells when enabled, or the compact 8-spell limit when disabled. |
| Show Spell Tooltips | Toggles Blizzard's native tooltip on breakdown spell rows. |
...
Selecting a specific window also exposes its own behavior settings:

- **Auto Current on Combat** -- switches a historical segment back to Current on pull; Overall remains selected.
- **Sync Segment Selection** -- applies a segment choice to every window that also has sync enabled.
- **Auto Swap Current/Overall** -- swaps this window for Mythic+ start/completion.
- **Default Meter Type on M+ Start** -- optionally selects one of the 11 meter types when a key starts.
- **Hide Timer** -- hides this window's header timer.
...
- **Behavior:** Number Format, Icon Style, and Show Secondary Value are override-aware appearance fields. Visibility, refresh rates, hover/detail options, pinned self, and breakdown position stay global. Combat/session automation and timer visibility live directly on each window.
